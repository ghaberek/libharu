...
procedure show_stripe_pattern( HPDF_Page page, atom x, atom y )
...
function main( integer argc, sequence argv )
	...
	sequence samp_text = "abcdefgABCDEFG123!#$%&+-@?"
	sequence samp_text2 = "The quick brown fox jumps over the lazy dog."
	atom tw
	atom fsize
	sequence buf
	integer len
	...
	*/
	...
	while fsize < 60 do
		...
		len = HPDF_Page_MeasureText( page, samp_text,
		                HPDF_Page_GetWidth(page) - 120, HPDF_FALSE )
	...
	for i = 1 to len do
		...
		HPDF_Page_ShowText( page, {samp_text[i]} )
	end for
